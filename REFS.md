- Bootloader Basics:
    - [Making Simple Bootloader using x86 Assembly (Nir Lichtman)](https://www.youtube.com/watch?v=xFrMXzKCXIc)
    - [Writing Boot Sector Code - By Susam Pal](https://susam.net/writing-boot-sector-code.html)
    - [Writing a Bootloader from Scratch - CS CMU](https://www.cs.cmu.edu/~410-s07/p4/p4-boot.pdf)
    - [OSDev - Bootloader](http://wiki.osdev.org/Bootloader)
    - [OSDev - Rolling Your Own Bootloader](http://wiki.osdev.org/Rolling_Your_Own_Bootloader)


- CPU Addressing Modes:
    - [Wikipedia - Real Mode](https://en.wikipedia.org/wiki/Real_mode)
    - [Addressing Modes - CS Emory](https://www.cs.emory.edu/~cheung/Courses/255/Syllabus/9-Intel/cs255/addressing-modes.html)
    - [BrokenThorn - Protected Mode](https://www.brokenthorn.com/Resources/OSDev8.html)
    - [OSDev - Real Mode](https://wiki.osdev.org/Real_Mode)
    - [OSDev - Protected Mode](https://wiki.osdev.org/Protected_Mode)
    - [OSDev - Memory Map (x86)](https://wiki.osdev.org/Memory_Map_(x86))


- IBM PS/2 Manuals:
    - [IBM Personal System/2 and Personal Computer - BIOS Manual](https://bitsavers.trailing-edge.com/pdf/ibm/pc/ps2/PS2_and_PC_BIOS_Interface_Technical_Reference_Apr87.pdf)


- Intel Manuals:
    - [Intel 64 and IA-32 Manual](https://www.intel.com/content/www/us/en/developer/articles/technical/intel-sdm.html)
    - [Intel 80386 Manual](https://pdos.csail.mit.edu/6.828/2018/readings/i386.pdf)
    - [Intel's System Developer Manual](http://www.cs.cmu.edu/~410-s07/doc/intel-sys.pdf)


- BIOS Interrupts:
    - [Ralf Brown's Interrupt List - Home](https://www.ctyme.com/rbrown.htm)
    - [Ralf Brown's Interrupt List - int 10h](https://www.ctyme.com/intr/rb-0106.htm)
    - [Stanislavs - INT 0x13, AH=0x02](https://www.stanislavs.org/helppc/int_13-2.html)
    - [Wikipedia - int 10h - Video](https://en.wikipedia.org/wiki/INT_10H)
    - [Wikipedia - int 13h - Diskette/Fixed Disk](https://en.wikipedia.org/wiki/INT_13H)
    - [Wikipedia - int 16h - Keyboard](https://en.wikipedia.org/wiki/INT_16H)
    - [OSDev - Ralf Brown's Interrupt List](http://wiki.osdev.org/Ralf_Brown's_Interrupt_List)
    - [Wikipedia - BIOS Color Attributes](https://en.wikipedia.org/wiki/BIOS_color_attributes)


- Global Descriptor Table (GDT):
    - [Wikipedia - GDT](https://en.wikipedia.org/wiki/Global_Descriptor_Table)
    - [Bren's Kernel Development - GDT](http://www.osdever.net/bkerndev/Docs/gdt.htm)
    - [OSDev - GDT](https://wiki.osdev.org/Global_Descriptor_Table)
    - [OSDev - GDT Tutorial](https://wiki.osdev.org/GDT_Tutorial)


- FAT File System:
    - [A tutorial on how to hardcode a FAT12 Header for a bootdisk.](https://board.flatassembler.net/topic.php?p=65958)
    - [Elm Chan: FAT's MAP](https://elm-chan.org/docs/rc/fat_map.png)
    - [Elm Chan: FAT Docs](https://elm-chan.org/docs/fat_e.html)
    - [FAT's Root Directory Entry Structure](https://www.szyunze.com/wp-content/uploads/2024/07/Root-Directory-SFN-Entry-Data-Structure.webp)


- Switching Back To Real Mode From Protected Mode:
    - [SCS Stanford](https://www.scs.stanford.edu/05au-cs240c/lab/i386/s14_05.htm)
    - [Stack Overflow](https://stackoverflow.com/questions/59894016/switch-from-protected-mode-to-real-mode)
    - [Stack Exchange](https://retrocomputing.stackexchange.com/questions/32242/is-it-possible-to-switch-from-32-bit-protected-mode-to-real-mode-without-going-t)
    - [Sudley Place](https://www.sudleyplace.com/pmtorm.html)
    - [OSDev](https://wiki.osdev.org/Real_Mode)
    - [Stack Overflow](https://retrocomputing.stackexchange.com/questions/32242/is-it-possible-to-switch-from-32-bit-protected-mode-to-real-mode-without-going-t)
    - [Gemini - Saving IVT](https://share.google/aimode/VTn8htXoatm8g50qy)


- Booting Linux:
    - [Linux Kernel Initialization](https://glennastory.net/boot/linux.html)
    - [Using the initial RAM disk (initrd)](https://www.kernel.org/doc/html/v4.14/admin-guide/initrd.html)
    - [OSDev - Boot Sequence](https://wiki.osdev.org/Boot_Sequence)
    - [OSDev - System Initialization (x86)](http://wiki.osdev.org/System_Initialization_(x86))
    - [Kernel Docs about linux boot](https://elixir.bootlin.com/linux/v7.0.11/source/Documentation/arch/x86/boot.rst)


- QEMU Stuff:
    - [qemu-img man page](https://manpages.debian.org/testing/qemu-utils/qemu-img.1.en.html)
    - [qemu block drivers reference](https://www.qemu.org/docs/master/system/qemu-block-drivers.html)
    - [Stack Overflow - Booting from Floppy Drive With QEMU](https://stackoverflow.com/questions/19961095/os-development-booting-from-floppy-drive-using-qemu)

- GRUB:
    - [Wikipedia - GNU GRUB](https://en.wikipedia.org/wiki/GNU_GRUB)


